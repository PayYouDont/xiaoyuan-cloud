...
public class PermitAllUrlProperties implements InitializingBean {
	...
	public void afterPropertiesSet() {
		...
		map.keySet().forEach(info -> {
			...
			Optional.ofNullable(method).ifPresent(inside -> info.getPatternsCondition().getPatterns()
			...
			Optional.ofNullable(controller).ifPresent(inside -> info.getPatternsCondition().getPatterns()
							.forEach(url -> releaseUrls.add(ReUtil.replaceAll(url, PATTERN, "*"))));
		});
